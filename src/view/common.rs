...
pub fn get_save_path(title: &str, kind: &str, context: &mut Context) -> (String, Option<usize>) {
    let mut path = format!("{}  {}.{}", title.replace('/', "-"),
                           Local::now().format("%Y%m%d-%H%M%S"),
                           kind);

    let library_index = match context.settings.save_to_library {
        Some(ref name) => {
            if name.is_empty() {
                None
            } else {
                let index = context.settings.libraries.iter().position(|x| x.name == *name);
                if index.is_some() { index } else { Some(0) }
            }
        }
        None => {
    ...
    // prepend library's path to file name
